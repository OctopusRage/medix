defmodule MedixWeb.BoardLive.FormComponent do
  ...
  def update(%{board: board} = assigns, socket) do
    changeset = Queue.change_board(board)
  ...
  def handle_event("validate", %{"board" => board_params}, socket) do
    changeset =
      ...
      |> Queue.change_board(board_params)
  ...
  defp save_board(socket, :edit, board_params) do
    case Queue.update_board(socket.assigns.board, board_params) do
      {:ok, board} ->
  ...
  defp save_board(socket, :new, board_params) do
    case Queue.create_board(board_params) do
      {:ok, board} ->
        notify_parent({:saved, board})
